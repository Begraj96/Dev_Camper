...
export class RegisterComponent implements OnInit {
  ...
  constructor(private _http: HttpClient, private router: Router, private toast: ToastrService, private authService: AuthService) { }
     ...
     // console.log('ok')
      this.authService.getregisterData(name, email, password, password2 )
        .subscribe((response) => {
          ...
          this.toast.success('Registration successfully!!!', 'success message')
          this.registerForm.reset();
          this.router.navigate(['login']);
        }, ({ error }) => {
          this.toast.error(error.email, 'Error message')
          console.log(error.email)
          this.errors = error.email
        })
    } else {
      this.confirmPass = 'inline'
      this.toast.error('Password does not match!!!', 'Error message')
    }
  };
